...
export function xpDedupeKey(uid: string, moduleId: string, type: XpEventType): string {
  return `${uid}:${moduleId}:${type}`;
}

/**
 * Idempotent XP award. Returns the event if this was a new award, or null if
 * an event with the same (uid, moduleId, type) already existed -- guarding
 * against duplicate XP from refreshes/resubmissions/retries.
 */
export async function awardXp(params: {
  ...
  const event: XpEvent = {
    id: xpDedupeKey(params.uid, params.moduleId, params.type),
    uid: params.uid,
